    ...
    >
      <b-form>
        <b-form-group
          ...
          invalid-feedback="Standard delivery charges is required."
        >
         <b-form-input
            id="name-input"
            ref="std"
            placeholder="Enter standard delivery charges here."
            required
            @keyup="CheckStdCharges()"
            v-model="myObj.DeliveryCharge.standardCharges"
          />
        </b-form-group>
           <b-form-group
          label="Express delivery charges"
          invalid-feedback="Express delivery charges is required."
        >
          <b-form-input
            id="name-input"
            ref="exd"
            placeholder="Enter express delivery charges here."
            required
            @keyup="CheckExpDCharges()"
            v-model="myObj.DeliveryCharge.expressDelivery"
          />
        </b-form-group>
        <b-form-group
          label="Select Cities"
            ...
            <div class="d-flex align-items-center justify-content-start">
              <div style="padding-right: 15px">
                <b-button @click="OpenAddModal()" variant="primary">
                  ...
                  <span class="text-nowrap">Add Delivery Charges</span>
                </b-button>
              </div>
              <!-- <b-form-input
                v-model="searchQuery"
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        '#',
        { label: 'Standard Charges', key: 'standardCharges' },
        { label: 'Express Delivery Charges ', key: 'expressDelivery' },
        'actions',
      ],
      items: [],
      selected: null,
 ...
 var elem = this.$refs['std'];
      const regex = /^\d+(,\d{1,2})?$/;
      if (parseInt(this.myObj.DeliveryCharge.standardCharges) !== 0) {
        if (regex.test(this.myObj.DeliveryCharge.standardCharges)) 
        {
            this.myObj.DeliveryCharge.standardCharges =parseInt(this.myObj.DeliveryCharge.standardCharges) ;
          return (elem.state = true);
        } else {
          return (elem.state = false);
        }
      } else {
       this.myObj.DeliveryCharge.standardCharges = '';
        return (elem.state = false);
      }
    },
    CheckExpDCharges()
    {
var elem = this.$refs['exd'];
      const regex = /^\d+(,\d{1,2})?$/;
      if (parseInt(this.myObj.DeliveryCharge.expressDelivery) !== 0) {
        if (regex.test(this.myObj.DeliveryCharge.expressDelivery)) 
        {
            this.myObj.DeliveryCharge.expressDelivery =parseInt(this.myObj.DeliveryCharge.expressDelivery) ;
          return (elem.state = true);
        } else {
          return (elem.state = false);
        }
      } else {
       this.myObj.DeliveryCharge.expressDelivery = '';
        return (elem.state = false);
      }
    },
      CheckVAT() {
      ...
      if (parseInt(this.myObj.country.vat) !== 0) {
        if (regex.test(this.myObj.country.vat)) 
        {
 ...
 var elem = this.$refs['cityId'];
    ...
    OpenAddModal() {
      ...
      this.myObj.DeliveryCharge.id = 0;
      this.myObj.DeliveryCharge.expressDelivery = '';
      this.myObj.DeliveryCharge.standardCharges = '';
      this.myObj.cityIds = [];
    ...
    OpenEditModal(row) {
      this.$bvModal.show('modal-login');
      this.myObj.DeliveryCharge.id = row.id;
      this.myObj.DeliveryCharge.expressDelivery = row.expressDelivery;
      this.myObj.DeliveryCharge.standardCharges = row.standardCharges;
    ...
    LoadData() {
      ...
      var categories = {
        method: 'get',
        url: 'https://localhost:44355/api/DeliveryCharges/LoadDeliveryCharges',
        headers: {
          Authorization: 'bearer ' + this.$store.state.userData.token,
        },
      };
      ...
      axios(categories)
        .then((response) => {
          console.log(response.data);
          this.items = response.data.deliveryCharges;
